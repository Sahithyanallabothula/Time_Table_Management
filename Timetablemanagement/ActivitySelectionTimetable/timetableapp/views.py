        ...
        if len(sectioncourses) > 0:
            if len(sectionrooms) > 0:
                if Activity.objects.filter(class_id=id).count() != 0:
                    if Activity.objects.filter(activity_type='Replaceable'):
                        deleteActivities(id)
                            ...
                            while j < courseLecs:
                                ...
                                if DupNum < workingHours + 5:
                                    if DupNum < 5:
                                        ...
                                        start_list=[]
                                        for i in range(section.start_time,section.end_time-1):
                                            if i==12:
                                                continue
                                            start_list.append(i)
                                        lecStartTime=random.choice(start_list)
                                        #lecStartTime=random.choice([i for i in range(section.start_time, section.end_time - 1) if i !=12])
                                        #s=(range(section.start_time,12))+(range(13,section.end_time - 1))
                                        #lecStartTime=random.choice(s)
